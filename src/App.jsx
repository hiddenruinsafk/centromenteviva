...
.nav-btn { padding:12px 22px; font-size:10.5px; letter-spacing:.1em; text-transform:uppercase; cursor:pointer; background:none; border:none; font-family:var(--sans); font-weight:500; color:rgba(255,255,255,.8); transition:color var(--t); white-space:nowrap; border-bottom:2px solid transparent; flex-shrink:0; }
...
function InteractiveMap({ address }) {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      // Coordinates for Via Armando Diaz 62, Casoria, Napoli, Italia
      const lat = 40.8614;
      const lng = 14.2868;
...
function BookingForm({ config }) {
  ...
  const submit = async () => {
    ...
    setTimeout(() => { setDone(false); setForm({ name:"", email:"", phone:"", service:"", date:"", time:"", message:"" }); setErrors({}); }, 4000);
  };
    ...
    <div>
      <div className="b-grid">
        ...
        <div>
          ...
          <input className="b-field" type="time" value={form.time} onChange={upd("time")} />
        </div>
      </div>
      ...
      <header className={`hdr${scrolled?" on":""}`}>
        <div className="hdr-inner">
          <div className="logo-w" onClick={onLogoClick}>
            <img src={config?.logo || LOGO} alt="Logo" style={{ height: scrolled ? 38 : 52, objectFit:"contain", transition:"height var(--t)", display:"block" }} />
          </div>
          <nav className="nav">
            {visible.map(s => (
...
function AdminSections({ sections, onSave, onNew, onEdit }) {
  const visible = sections.filter(s => !s.builtin);
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <button onClick={onNew} className="btn btn-d" style={{ width: "100%" }}>+ Nuova Sezione</button>
      {visible.map(sec => (
        <div key={sec.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 12, background: "var(--bg)", borderRadius: 4 }}>
          <div style={{ flex: 1 }}>
...
function AdminSettings({ config, onSave }) {
  const [f, setF] = useState({ name:"", address:"", phone:"", email:"", adminPassword:"", ownerEmail:"", svcId:"", tplId:"", pubKey:"", services: DEFAULT_SERVICES, social: {}, logo: "", loadingImage: "", ...config, services: (config?.services || DEFAULT_SERVICES) });
