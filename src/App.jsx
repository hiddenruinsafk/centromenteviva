...
async function getAvailableSlots(config, selectedDate) {
  try {
    ...
    // Fetch events from Firestore (we'll store calendar events there)
    ...
    const [lunchStartH, lunchStartM] = config.lunchBreakStart.split(":").map(Number);
    ...
    for (let h = startH; h < endH; h++) {
      for (let m = 0; m < 60; m += 30) {
        ...
        if (!isBooked) {
          slots.push({
            time: slotStart.toLocaleTimeString("it-IT", { hour: "2-digit", minute: "2-digit", hour12: false }),
...
async function createCalendarEvent(config, form) {
  try {
    if (!config?.googleCalendarId) return false;
